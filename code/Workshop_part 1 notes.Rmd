...
## Tidyverse Package
The tidyverse package undoubtedly is the most important package for data analysis in R. It assemblies all the packages you need for your data-analysis. For this part of the workshop we will go through importing data, and data transformations. Parameter estimation and reporting will be in workshop part 2. The # sign is used to write notes; that is explain what each line is doing. It makes it easier to understand your code whn you revisit it say in month time.
...
Data wrangling is a datascience term for all the series of activities you will perform on your raw and sometimes messy dataset to transform it into a clean and tidy dataset for analysis. Today we will go through the three main data wrangling essentials for cleaning data.
...
We will just introduce you some of the tools in tidyverse, in particular the dplyr package, for data wrangling. I think the dplyr package is the most important too for data wrangling. 

### Data Selection
In this section we r going to select some variables from the original dataset (workshop_data) and name it as data1
...
### Data Mutate
Here are are going to create another column, the square of bodywt, from our existing column (bodywt). Again we have introduced another function called View(), which helps us to open the new data in a new window. 
